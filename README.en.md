...
- `scripts/install-computer-use-local.ps1`: Layered Windows Computer Use and Chrome runtime repair/verification. It accepts only user-local runtimes that match the current package, preserves unrelated TOML keys, avoids the WindowsApps Python alias, and checks a live Desktop pipe plus the Window object contract in strict mode.
- `scripts/probe-computer-use-runtime.mjs`: A redacted `sky.list_windows` runtime/window-contract probe that never prints window titles.
- `scripts/probe-computer-use-client.mjs`: A standard legacy-client probe that calls `setupComputerUseRuntime()` and then `sky.list_windows()` through either the live Desktop pipe or an explicit offline fixture, emitting only contract counts.
- `scripts/test-computer-use-config-preservation.ps1`: Isolated regression for TOML writes, unrelated-key preservation, UTF-8 without a BOM, and duplicate-key rejection; it does not touch live Desktop state.
- `scripts/test-computer-use-client-wrapper.ps1`: Offline regression proving the legacy wrapper setup and Window contract, including rejection of malformed `{app,id}` objects.
...
- `references/computer-use-e2e-validation.md`: Layered acceptance for local runtime, Desktop bootstrap, tool injection, window capture/action, browser smoke testing, and provider-native `computer_call` support.
...
This skill supports self-updating: before each substantive use, the agent first tries to check GitHub and sync the latest version, so you do not need to repeatedly return to GitHub and pull updates manually. A customized copy containing `.skill-local-overrides` reports available updates but refuses to overwrite local changes by default; pass `-Force` only after backing up and reviewing the diff. If the network is unavailable, GitHub cannot be reached, or the download fails, the update step is skipped and the agent should continue with the currently installed local version.
...
- `-StrictVerifyOnly` also reports the current package-matching runtime ID, `computer_use=true`, a trusted `code_mode_host`/`code_mode`, a live Desktop native pipe, and `windowContract=true` with no invalid Window objects. `-SkipDesktopPipeCheck` is for offline tests only and does not prove Desktop bootstrap.
- Legacy layouts additionally report `client wrapper ok (pipe)` after the standard `setupComputerUseRuntime()` path calls `sky.list_windows()` through the active pipe; when no pipe is requested, `VerifyOnly` uses only the explicit fixture mode and labels it accordingly.
- Run `scripts\test-computer-use-config-preservation.ps1 -TemporaryRoot <temporary-root>` from the downloaded copy for an isolated config-writer regression. It checks TOML semantics, UTF-8 without a BOM, unrelated-key preservation, and duplicate-key rejection without touching live Desktop state.
- Run `scripts\test-computer-use-client-wrapper.ps1 -TemporaryRoot <temporary-root>` for the isolated legacy-wrapper regression; it does not install a plugin or touch Desktop state.
- Correlate Desktop logs by current package path, PID/process UUID, and launch time. Current-process logs must retain expected descriptors and stop reporting `not_in_bundled_marketplace_plugin_names`, `bundled_plugins_reconcile_failed`, or marketplace source conflicts. Marker text echoed in prompts or tool commands is not log evidence.
- A passing strict check proves only the local/runtime/Desktop-pipe layers. Use `references/computer-use-e2e-validation.md` to record tool injection, screenshots, a real action, the `example.com` browser smoke test, and provider-native Computer Use separately; `list_windows` is not end-to-end proof.
